...
class SignalBot(object):
    ...
    def handle_message(self, msgid, sender, groupIDs, message, attachments):
        try:
            ...
            user = self.worker_input_queue.add_task_with_result_blocking(
                partial(self.aggregator.get_user_by_phone_number, phone_number),
                self.logger,
            )
            reply = self.worker_input_queue.add_task_with_result_blocking(
