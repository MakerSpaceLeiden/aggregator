...
class TelegramBot(object):
    ...
    def send_notification(self, user, notification, logger):
        ...
        logger.info(
            f"Sending notification of type {notification.__class__.__name__} to user {user.user_id} {user.full_name}"
        )
    ...
    def handle_message(self, bot, update):
        try:
            ...
            self.logger.info(
                f'Received message "{message}" from user {user.full_name if user else "<unregistered>"}'
            )
            if message.startswith("/start"):
                connection_token = get_connection_token_from_message(message)
                if connection_token:
                    user = self._make_new_telegram_association_for_user(
                        user, connection_token, telegram_id
                    )
            ...
            self.logger.exception("Unexpected exception in message handler")

    def _send_message(self, message, telegram_user_id):
        markdown = message.get_markdown()
        reply_markup = (
            ReplyKeyboardMarkup(
                [[command.text for command in message.next_commands]],
                one_time_keyboard=True,
            )
            if message.next_commands
            else ReplyKeyboardRemove()
        )
        self.updater.bot.send_message(
            telegram_user_id, markdown, reply_markup=reply_markup
        )
    ...
    def _make_new_telegram_association_for_user(
        self, current_user, connection_token, telegram_id
    ):
        return self.worker_input_queue.add_task_with_result_blocking(
            partial(
                self.aggregator.make_new_telegram_association_for_user,
                current_user,
                connection_token,
                telegram_id,
            ),
    ...
    def _get_user_by_telegram_id(self, telegram_id):
        return self.worker_input_queue.add_task_with_result_blocking(
            partial(self.aggregator.get_user_by_telegram_id, telegram_id), self.logger
        )

    def _handle_new_bot_conversation(self, chat_id, user, message):
        return self.worker_input_queue.add_task_with_result_blocking(
            partial(
                self.aggregator.handle_new_bot_conversation, chat_id, user, message
            ),
            self.logger,
        )
    ...
    def _handle_bot_message(self, chat_id, user, message):
        ...
        return self.worker_input_queue.add_task_with_result_blocking(
            partial(self.aggregator.handle_bot_message, chat_id, user, message),
            self.logger,
        )
