...
MISCELLANEOUS_ERRORS_IN_LOGS_3_MAR_2019_FILE_PATH = os.path.join(
    DIR_PATH, "errors_3_mar_2019.txt"
)
...
class TestMqttParsing(unittest.TestCase):
    def test_groteschakelaar(self):
        self.assertEqual(
            parse_message("makerspace/groteschakelaar", "1"), ("space_open", True)
        )
        self.assertEqual(
            parse_message("makerspace/groteschakelaar", "0"), ("space_open", False)
        )
        self.assertEqual(
            parse_message("makerspace/groteschakelaar/status/", "werkend"), ("ignore",)
        )
    ...
    def test_door(self):
        self.assertEqual(
            parse_message(
                "ac/log/master",
                'JSON={"ok": true, "userid": 22, "name": "Stefano Masini", "email": "stefano@stefanomasini.com", "machine": "spacedeur", "acl": "approved", "cmd": "energize"}',
            ),
        ...
        self.assertEqual(
            parse_message(
                "ac/log/master",
                'JSON={"ok": true, "userid": 22, "name": "Stefano Masini", "email": "stefano@stefanomasini.com", "machine": "spacedeur", "acl": "approved", "cmd": "leave"}',
            ),
            ("user_left_space", 22),
        )
    ...
    def test_tableSaw(self):
        self.assertEqual(
            parse_message(
                "ac/log/master",
                'JSON={"ok": true, "userid": 22, "name": "Stefano Masini", "email": "stefano@stefanomasini.com", "machine": "tablesaw", "acl": "approved"}',
            ),
        ...
        self.assertEqual(
            parse_message(
                "ac/log/tablesaw",
                "tablesaw Machine switched ON with the safety contacto green on-button.",
            ),
        ...
        self.assertEqual(
            parse_message(
                "ac/log/tablesaw",
                "tablesaw Machine switched OFF with the safety contactor off-button.",
            ),
    ...
    def test_planer(self):
        self.assertEqual(
            parse_message(
                "ac/log/master",
                'JSON={"ok": true, "userid": 22, "name": "Stefano Masini", "email": "stefano@stefanomasini.com", "machine": "planer", "acl": "approved"}',
            ),
            ("user_activated_machine", 22, "planer"),
        )
        self.assertEqual(
            parse_message(
                "ac/log/planer", "planer Green button on safety contactor pressed."
            ),
        ...
        self.assertEqual(
            parse_message(
                "ac/log/planer",
                "planer Switching off - red button at the back pressed.",
            ),
        ...
        for file in SAMPLE_FILES:
            ...
            for line in open(fullfile, encoding="utf-8", errors="ignore").readlines():
                ...
                self.assertIsNotNone(
                    parsed_message,
                    f"Unable to parse message: ({repr(topic)}, {repr(message)}) in {file} at line {lno}:{line}",
                )
                if (
                    parsed_message[0] == "machine_state"
                    and parsed_message[-1] != "ready"
                ):
    ...
    def test_miscellaneous_errors_in_logs_3_mar_2019(self):
        for line in open(MISCELLANEOUS_ERRORS_IN_LOGS_3_MAR_2019_FILE_PATH).readlines():
            topic, message = line.strip()[
                line.strip().index("Cannot parse message:") + 22 :
            ].split(" - ", 1)
            parsed_message = parse_message(topic, message)
            self.assertIsNotNone(
                parsed_message,
                f"Unable to parse message: ({repr(topic)}, {repr(message)})",
            )
