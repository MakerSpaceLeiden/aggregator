...
def run_http_server(
        ...
        def decorated(*args, **kwargs):
            try:
                if not request.authorization or not (
                    ...
                    and request.authorization.password == basic_auth["password"]
                ):
                    realm = basic_auth["realm"]
                    request.logger.error(
                        f'Wrong basic auth: username = {request.authorization.username if request.authorization else "n/a"}'
                    )
                    return Response(
    ...
    @with_basic_auth
    async def tags():
        _tags = await worker_input_queue.add_task_with_result_future(
            aggregator.get_tags, request.logger
        )
        return jsonify(
            {
                "tags": [
    ...
    async def telegram_token():
        ...
        token = await worker_input_queue.add_task_with_result_future(
            partial(
                aggregator.create_telegram_connect_token, request_payload["user_id"]
            ),
            request.logger,
        )
    ...
    async def notification_test():
        ...
        request_payload = json.loads(request_body)
        await worker_input_queue.add_task_with_result_future(
            partial(aggregator.send_notification_test, request_payload["user_id"]),
            request.logger,
        )
        return Response("Ok", mimetype="text/plain")
