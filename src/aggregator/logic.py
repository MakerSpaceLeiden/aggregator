...
class Aggregator(object):
    ...
    def _get_machine_by_name(self, machine_name, logger):
        ...
        if not machine:
            ...
            filtered_machines = [
                m for m in all_machines if m.node_machine_name == machine_name
            ]
    ...
    def user_entered_space(self, user_id, logger):
        ...
        self.redis_adapter.store_history_line(
            UserEntered(user_id, now, user.first_name, user.last_name), logger
        )
    ...
    def user_left_space(self, user_id, logger):
        ...
        users_still_in_space = self.redis_adapter.get_user_ids_in_space_with_timestamps(
            logger
        )
    ...
    def _user_leave_checks(self, user, is_last_user_leaving, logger):
        ...
        for machine_state in all_machines_states:
            if (
                machine_state["user"]
                and machine_state["user"]["user_id"] == user.user_id
            ):
                problems.append(
                    ProblemMachineLeftOnByUser(machine_state["machine"]["name"])
                )
            elif is_last_user_leaving:
                problems.append(
                    ProblemMachineLeftOnBySomeoneElse(machine_state["machine"]["name"])
                )
        ...
        if len(problems) > 0:
            self.send_user_notification(
                user,
                ProblemsLeavingSpaceNotification(
                    user, self.clock.now(), problems, is_last_user_leaving
                ),
                logger,
            )
    ...
    def is_user_id_in_space(self, user_id, logger):
        for (
            user_id_in_space,
            ts_checkin,
        ) in self.redis_adapter.get_user_ids_in_space_with_timestamps(logger):
    ...
    def check_expired_machine_state(self, logger):
        ...
        for machine in self.redis_adapter.get_machines_on(logger):
            ...
            if state_machine_on and not machine_state:
                logger.info(
                    "Machine {0} ON state expired. Cable must have been disconnected. Setting to off."
                )
    ...
    def get_space_state_for_json(self, logger):
        ...
        users = [
            (self._get_user_by_id(user_id, logger), ts_checkin)
            for user_id, ts_checkin in data
        ]
        ...
        return {
            "lights_on": [
                light.for_json() for light in ALL_LIGHTS if light.label in lights_on
            ],
            "space_open": self.redis_adapter.get_space_open(logger),
            "machines_on": all_machines_states,
            "machines": [
                self._get_machine_state(machine, logger) for machine in all_machines
            ],
            "history": [
                self._get_history_line_for_json(hl) for hl in all_history_lines
            ],
            "users_in_space": [
                {
                    "user": user.for_json(),
    ...
    def _get_machines_on_for_json(self, logger):
        all_machines_states = [
            self._get_machine_onoff_state(machine, logger)
            for machine in self.redis_adapter.get_machines_on(logger)
        ]
    ...
    def _get_machine_onoff_state(self, machine_name, logger):
        ...
        return {
            ...
            "ts_human": state["ts"].human_delta_from(self.clock.now())
            if state
            else None,
    ...
    def send_user_notification(self, user, notification, logger):
        ...
        if user.uses_email():
            try:
                ...
                logger.exception(
                    "Unexpected exception when trying to notify user via email"
                )
    ...
    def user_activated_machine(self, user_id, machine, logger):
        ...
        user = self._get_user_by_id(user_id, logger)
        logger.info(
            f"User {user.full_name if user else user_id} activated machine {machine}"
        )
    ...
    def machine_power(self, machine, state, logger):
        ...
        if state == "on":
            ...
            if not user_id:
                ...
                user = self._get_user_by_id(user_id, logger)
                logger.info(
                    f"User {user.full_name if user else user_id} turning on machine {machine}"
                )
                now = self.clock.now()
            ...
            if not state:
                logger.error(
                    f"Machine {machine} turned off without corresponding ON record"
                )
            else:
    ...
    def _warn_user_of_machine_left_on(self, machine_name, user_id, logger):
        user = self._get_user_by_id(user_id, logger)
        logger.info(
            f"Warning user {user.full_name if user else user_id} that machine {machine_name} was left on"
        )
        if user:
            machine = self._get_machine_by_name(machine_name, logger)
            self.send_user_notification(
    ...
    def get_users_seen_no_later_than_days(self, num_days, logger):
        ...
        return [
            self._get_user_by_id(user_id, logger)
